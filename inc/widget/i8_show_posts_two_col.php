...
class i8_show_posts_two_col extends WP_Widget
{
        ...
        <div class="i8-panel">
            ...
            <p>
                <input type="checkbox" name="<?php echo $this->get_field_name('hide_title'); ?>"
                    id="<?php echo $this->get_field_id('hide_title'); ?>" class="checkbox" <?php echo ($hide_title == true) ? 'checked="checked"' : ''; ?>>
            ...
            <p>
                <label for="<?php echo $this->get_field_id('sub_title'); ?>">زیر عنوان (متنی که در زیرعنوان قرار می
                    گیرد)</label>
            ...
            <p>
                ...
                <input type="text" name="<?php echo $this->get_field_name('title2'); ?>"
                    id="<?php echo $this->get_field_id('title2'); ?>" value="<?php echo $title2; ?>" class="widefat">
            </p>
            <p>
                <label for="<?php echo $this->get_field_id('sub_title2'); ?>">زیر عنوان (متنی که در زیرعنوان قرار می
                    گیرد)</label>
                <input type="text" name="<?php echo $this->get_field_name('sub_title2'); ?>"
                    id="<?php echo $this->get_field_id('sub_title2'); ?>" value="<?php echo $sub_title2; ?>" class="widefat">
            </p>
            <p>
                <label for="<?php echo $this->get_field_id('icon'); ?>">آیکن ستون اول( کد آیکن بوت استرپ)</label>
                <input type="text" name="<?php echo $this->get_field_name('icon'); ?>"
                    id="<?php echo $this->get_field_id('icon'); ?>" value="<?php echo $icon; ?>" class="widefat">
            </p>
            <p>
                <label for="<?php echo $this->get_field_id('icon_img'); ?>">تصویر آیکن ستون اول(آدرس تصویر)</label>
                <input type="text" name="<?php echo $this->get_field_name('icon_img'); ?>"
                    id="<?php echo $this->get_field_id('icon_img'); ?>" value="<?php echo $icon_img; ?>" class="widefat">
            </p>
            <p>
                <label for="<?php echo $this->get_field_id('icon2'); ?>">آیکن ستون دوم( کد آیکن بوت استرپ)</label>
                <input type="text" name="<?php echo $this->get_field_name('icon2'); ?>"
                    id="<?php echo $this->get_field_id('icon2'); ?>" value="<?php echo $icon2; ?>" class="widefat">
            </p>
            <p>
                <label for="<?php echo $this->get_field_id('icon_img2'); ?>">تصویر آیکن ستون دوم(آدرس تصویر)</label>
                <input type="text" name="<?php echo $this->get_field_name('icon_img2'); ?>"
                    id="<?php echo $this->get_field_id('icon_img2'); ?>" value="<?php echo $icon_img2; ?>" class="widefat">
            </p>
        </div>
        <div class="i8-panel">
            ...
            <p>
                ...
                <input type="number" name="<?php echo $this->get_field_name('num'); ?>"
                    id="<?php echo $this->get_field_id('num'); ?>" value="<?php echo (!empty($num)) ? $num : 5; ?>"
                    class="widefat">
            </p>
        </div>
        <div class="i8-panel">
            ...
            <p>
                <input type="checkbox" name="<?php echo $this->get_field_name('show_desktop'); ?>"
                    id="<?php echo $this->get_field_id('show_desktop'); ?>" class="checkbox" <?php echo ($show_desktop == true) ? 'checked="checked"' : ''; ?>>
                <label for="<?php echo $this->get_field_id('show_desktop'); ?>">نمایش دسکتاپ</label>
            </p>
            <p>
                <input type="checkbox" name="<?php echo $this->get_field_name('show_mobile'); ?>"
                    id="<?php echo $this->get_field_id('show_mobile'); ?>" class="checkbox" <?php echo ($show_mobile == true) ? 'checked="checked"' : ''; ?>>
    ...
    function widget($args, $instance)
    {
        ...
        $primary_cat_url2 = get_category_link(intval($cat2));
        $title2 = '<a href="' . $primary_cat_url2 . '" style="color:' . $cat_color2 . '" > ' . apply_filters('wp_widget_title', $instance['title2']) . '</a>';
        ...
        <?php
        // The Query
        $simple_post_list_query = new WP_Query(
            array(
                'posts_per_page' => $num,
                'cat' => $cat,
                'order' => 'DESC',
        ...
        $simple_post_list_query2 = new WP_Query(
            array(
                'posts_per_page' => $num,
                'cat' => $cat2,
                'order' => 'DESC',
        ...
        echo '<div class="row me-xl-0 row-gap-2">';
        i8_show_post_list_creator($simple_post_list_query, $cat, $cat_color, $title, $sub_title_print, $icon_print, $num, $args, true);
        i8_show_post_list_creator($simple_post_list_query2, $cat2, $cat_color2, $title2, $sub_title_print2, $icon_print2, $num, $args, false);
        echo '</div>';
        ?>
...
function i8_show_post_list_creator($simple_post_list_query, $cat, $cat_color, $title, $sub_title_print, $icon_print, $num, $args, $first_child = false)
{
    $row = '<div class="col-xl-12 col-md-12 col-sm-12 col-24 px-0 px-sm-2 ';
    $row .= ($first_child) ? 'pe-xl-0' : 'ps-xl-0';
    $row .= '"><div class="d-flex flex-column row-gap-3 ">';
    echo $row;

    if ($title) {
        echo '<div class="title-icon d-flex align-items-center align-items-center">';
        echo '<p class="text-title display-3 fw-4 me-2 mb-0" style="color:' . $cat_color . '" >';
    ...
    if ($simple_post_list_query->have_posts()) {
        while ($simple_post_list_query->have_posts()) {
            ...
            ?>
            <div class="vitrin-2-small-item1  position-relative d-flex justify-content-center flex-row p-0">
                <a href="<?php the_permalink(); ?>" class="image_frame">
                    <?php echo i8_the_thumbnail('i8-420-256', 'hover hover round-15px multi-item-thumb w-100 i8-img-fit ', $dimenition = array('width' => 408, 'height' => 256), true, '', false, true); ?>
                </a>
    ...
    if ($category_posts->have_posts()) { ?>
        ...
        while ($category_posts->have_posts()) {
            ...
            <div
                class="col-24  mini-article d-flex   <?php echo ($category_posts->current_post + 1 == $category_posts->post_count) ? '' : 'border-bottom'; ?> px-0 pb-3 ">
                <div>
                    <a href="<?php the_permalink(); ?>" class="image_frame">
                ...
                <div class="d-flex flex-column ">
                    <h3 class="me-2 l22-05 post-title">
                        <a class=" display-4 l1" href="<?php echo get_the_permalink(); ?>">
                            <?php i8_limit_text(get_the_title(), 88, '...'); ?>
                        </a>
                    </h3>
                </div>
            </div>
