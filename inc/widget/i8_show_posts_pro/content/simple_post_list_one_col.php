...
if ($category_posts->have_posts()) {
    while ($category_posts->have_posts()) {
        ...
        <?php if ($hide_thumb != 'on'): ?>
            <div
                class="<?php echo $col; ?> d-flex mb-3 align-items-start gap-2 align-items-start  px-0  pb-3 <?php echo ($category_posts->current_post + 1 == $category_posts->post_count) ? '' : 'border-bottom'; ?>">
                <div>
                    <a href="<?php the_permalink(); ?>" class="image_frame">
                        <?php echo i8_the_thumbnail('i8-80-63', 'hover ' . $thumb_radius, array("width" => $thumb_width , "height" => $thumb_height)); ?>
                    </a>
                </div>
