...
function i8_custom_pagination()
{
    ...
    if ($paginate_links) {
        ?>
        <div class="row  d-flex align-content-center justify-content-center box-style-white p-2 box-style-white p-2 mx-0">
...
function render_primary_category_meta_box($post)
{
    ...
    <?php
    $post_link1_id = get_post_meta($post->ID, 'hasht-related-link1', true);
    $post_link1 = ($post_link1_id) ? get_permalink($post_link1_id) : '';
    $post_link2_id = get_post_meta($post->ID, 'hasht-related-link2', true);
    $post_link2 = ($post_link2_id) ? get_permalink($post_link2_id) : '';
    $post_link3_id = get_post_meta($post->ID, 'hasht-related-link3', true);
    $post_link3 = ($post_link3_id) ? get_permalink($post_link3_id) : '';


    ?>
    <!-- لینک پست مرتبط ۱ -->
    <div id="hasht-related-link1-sec" class="misc-pub-section " <?php echo ($selected_post_structure != 'hot') ? ' style="display:none;" ' : ''; ?>>
        <label for="hasht-related-link1">لینک پست مرتبط اول</label>
        <input type="text" name="hasht-related-link1" id="hasht-related-link1" class="widefat"
            value="<?php echo $post_link1; ?>">
        <?php if ($post_link1_id): ?>
            <a href="<?php echo $post_link1; ?>" target="_blank">
                <?php echo get_the_title($post_link1_id); ?>
            </a>
        <?php endif; ?>
    </div>

    <!-- لینک پست مرتبط ۲ -->
    <div id="hasht-related-link2-sec" class="misc-pub-section " <?php echo ($selected_post_structure != 'hot') ? ' style="display:none;" ' : ''; ?>>
        <label for="hasht-related-link2">لینک پست مرتبط دوم</label>
        <input type="text" name="hasht-related-link2" id="hasht-related-link2" class="widefat"
            value="<?php echo $post_link2; ?>">
        <?php if ($post_link2_id): ?>
            <a href="<?php echo $post_link2; ?>" target="_blank">
                <?php echo get_the_title($post_link2_id); ?>
            </a>
        <?php endif; ?>
    ...
    <!-- لینک پست مرتبط ۳ -->
    <div id="hasht-related-link3-sec" class="misc-pub-section " <?php echo ($selected_post_structure != 'hot') ? ' style="display:none;" ' : ''; ?>>
        <label for="hasht-related-link3">لینک پست مرتبط سوم</label>
        <input type="text" name="hasht-related-link3" id="hasht-related-link3" class="widefat"
            value="<?php echo $post_link3; ?>">
        <?php if ($post_link3_id): ?>
            <a href="<?php echo $post_link3; ?>" target="_blank">
                <?php echo get_the_title($post_link3_id); ?>
            </a>
        <?php endif; ?>
        ...
        jQuery(document).ready(function ($) {
            ...
            $("#i8_post_structure").change(function () {
                ...
                if (selectedValue === "video") {
                    ...
                    $("#hasht-related-link1-sec").hide();
                    $("#hasht-related-link2-sec").hide();
                    $("#hasht-related-link3-sec").hide();
                } else if (selectedValue === "hot") {
                    $("#hasht-related-link1-sec").show();
                    $("#hasht-related-link2-sec").show();
                    $("#hasht-related-link3-sec").show();
...
function save_primary_category_meta_data($post_id)
{
    ...
    if (isset($_POST['hasht-related-link1'])) {
        $post1_id = url_to_postid($_POST['hasht-related-link1']);
        update_post_meta($post_id, 'hasht-related-link1', $post1_id);

    }

    if (isset($_POST['hasht-related-link2'])) {
        $post2_id = url_to_postid($_POST['hasht-related-link2']);
        update_post_meta($post_id, 'hasht-related-link2', $post2_id);
    }

    if (isset($_POST['hasht-related-link3'])) {
        $post3_id = url_to_postid($_POST['hasht-related-link3']);
        update_post_meta($post_id, 'hasht-related-link3', $post3_id);
    }
...
function i8_primary_category($post_id, $string_return = false)
{
    ...
    $cat_color = get_term_meta($primary_cat_id[0], 'i8_CustomTerm_color', true);
    $cat_color = isset($cat_color) ? get_term_meta($primary_cat_id[0], 'i8_CustomTerm_color', true) : '';
    ...
    if (@$primary_cat_id[0] != ''):
        ...
        if (!$string_return):
            return '<a class="post-category display-5" href="' . $primary_cat_url . '" style="color:' . $cat_color . ' !important; " >' . $primary_cat_name . "</a>";
        else:
    ...
    foreach ($items as $item) {
        if ($item->menu_item_parent == $parent_id) {
            ...
            if (!empty($submenu)) {
                ...
                $menu .= '<a class="nav-link  hasht-dropdown-toggle " id="navbarDropdown" role="button" data-bs-toggle="dropdown" aria-expanded="false" href="' . esc_url($item->url) . '">' . esc_html($item->title) . '<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" fill="currentColor" class="bi bi-chevron-down mx-1" viewBox="0 0 24 24">
...
function build_custom_menu_by_id($menu_id, $style_type = 'row')
{
    ...
    if ($menu_items) {
        ...
        $gap = ($style_type == 'column') ? 'gap-0' : 'gap-sm-3 gap-0';
    ...
    if (is_home() || is_front_page()) {
        ...
        if (is_single()) {
            if ($primary_cat = i8_primary_category(get_the_ID(), true)):
                ...
                echo '<a class=""  href="' . $primary_cat['cat_url'] . '" style="color:' . $cat_color . ';">' . $primary_cat['cat_name'] . '</a>';
            else:
                // the_category('/ ');
                // echo $delimiter;
            endif;
 ...
 */
function i8_mobile_menu($location, $custom_class = ' ', $icon_size = array('32', '32'))
{
    ...
    $menu = '';
    echo '<nav class="i8-h-menu ' . $custom_class . ' ">
            <input type="checkbox" id="menu" name="menu" class="m-menu__checkbox">
            <label class="m-menu__toggle" for="menu">
            <?xml version="1.0" encoding="UTF-8"?><svg  width="' . $icon_size[0] . 'px" height="' . $icon_size[1] . 'px" stroke-width="1.5" viewBox="0 0 24 24" fill="none" xmlns="http://www.w3.org/2000/svg" color="var(--i8-light-fg-color)"><path d="M8 6h12M4 6.01l.01-.011M4 12.01l.01-.011M4 18.01l.01-.011M8 12h12M8 18h12" stroke="var(--i8-light-fg-color)" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round"></path></svg>
            </label>
             <div class="m-menu">
        ...
        <div class="d-none d-xl-flex d-lg-flex d-md-flex justify-content-center gap-2 social-links ">
            <?php if ($twitter): ?>
                <a class="p-0 p-lg-0 p-sm-1 dark-btn" target="_blank" href="<?php echo $twitter; ?>" alt="twitter share button"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="<?php echo $width; ?>" height="<?php echo $height; ?>"
                        fill="var(--i8-light-secondary)" class="bi svg-icon bi-twitter mx-1"
            ...
            <?php if ($facebook): ?>
                <a class="p-0 p-lg-0 p-sm-1 dark-btn" target="_blank" href="<?php echo $facebook; ?>"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="<?php echo $width; ?>" height="<?php echo $height; ?>"
                        fill="var(--i8-light-secondary)" class="bi svg-icon bi-facebook mx-1"
                        viewBox="0 0 <?php echo $width; ?> <?php echo $height; ?>">
                        <path
            ...
            <?php if ($telegram): ?>
                <a class="p-0 p-lg-0 p-sm-1 dark-btn" href="<?php echo $telegram; ?>" alt="telegram share button"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="<?php echo $width; ?>" height="<?php echo $height; ?>"
                        fill="var(--i8-light-secondary)" class="bi svg-icon bi-telegram mx-1"
                        viewBox="0 0 <?php echo $width; ?> <?php echo $height; ?>">
                        <path
            ...
            <?php if ($whatsapp): ?>
                <a class="p-0 p-lg-0 p-sm-1 dark-btn" target="_blank" href="<?php echo $whatsapp; ?>"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="<?php echo $width; ?>" height="<?php echo $height; ?>"
                        fill="var(--i8-light-secondary)" class="bi svg-icon bi-whatsapp mx-1"
                        viewBox="0 0 <?php echo $width; ?> <?php echo $height; ?>">
                        <path
            ...
            <?php if ($instagram): ?>
                <a class="p-0 p-lg-0 p-sm-1 dark-btn" target="_blank" href="<?php echo $instagram; ?>"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="<?php echo $width; ?>" height="<?php echo $height; ?>"
                        fill="var(--i8-light-secondary)" class="bi svg-icon bi-instagram mx-1"
                        viewBox="0 0 <?php echo $width; ?> <?php echo $height; ?>">
                        <path
            ...
            <?php if ($youtube): ?>
                <a class="p-0 p-lg-0 p-sm-1 dark-btn" target="_blank" href="<?php echo $youtube; ?>" alt="youtube share button"
                    ...
                    <svg xmlns="http://www.w3.org/2000/svg" width="<?php echo $width; ?>" height="<?php echo $height; ?>"
                        fill="var(--i8-light-secondary)" class="bi svg-icon bi-youtube mx-1"
                        viewBox="0 0 <?php echo $width; ?> <?php echo $height; ?>">
                        <path
            ...
            <?php if ($aparat): ?>
                <a class="p-0 p-lg-0 p-sm-1 dark-btn" target="_blank" href="<?php echo $aparat; ?>" alt="aparat share button"
                    ...
                    <svg fill="var(--i8-light-secondary)" width="<?php echo $width; ?>" height="<?php echo $height; ?>"
                        class="bi svg-icon bi-twitter mx-1" viewBox="0 0 24.00 24.00" role="img"
                        xmlns="http://www.w3.org/2000/svg" transform="rotate(0)matrix(1, 0, 0, 1, 0, 0)"
                        stroke="var(--i8-light-secondary)" stroke-width="0.00024000000000000003">
                        <g id="SVGRepo_bgCarrier" stroke-width="0"></g>
                        <g id="SVGRepo_tracerCarrier" stroke-linecap="round" stroke-linejoin="round"
                            stroke="var(--i8-light-secondary)" stroke-width="0.192"></g>
            ...
            <?php if ($eitta): ?>
                <a class="p-0 p-lg-0 p-sm-1 dark-btn" target="_blank" href="<?php echo $eitta; ?>" alt="eitta share button"
                    ...
                    <svg fill="var(--i8-light-secondary)" width="<?php echo $width; ?>" height="<?php echo $height; ?>"
                        viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" class="bi svg-icon ">
                        <path
            ...
            <?php if ($bale): ?>
                <a class="p-0 p-lg-0 p-sm-1 dark-btn" target="_blank" href="<?php echo $bale; ?>" alt="bale share button"
                    ...
                    <svg fill="var(--i8-light-secondary)" width="<?php echo $width; ?>" height="<?php echo $height; ?>"
                        role="img" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg" class="bi svg-icon ">
                        <path
            ...
            <?php if ($rubika): ?>
                <a class="p-0 p-lg-0 p-sm-1 dark-btn" target="_blank" href="<?php echo $rubika; ?>" alt="rubika share button"
                    ...
                    <svg version="1.1" id="Capa_1" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink"
                        x="0px" y="0px" fill="var(--i8-light-secondary)" width="<?php echo $width; ?>"
                        height="<?php echo $height; ?>" viewBox="0 0 412.287 412.287"
                        style="enable-background:new 0 0 412.287 412.287;" xml:space="preserve" class="bi svg-icon ">
                        <g>
...
function insert_related_post_link_in_content($content)
{
    ...
    if ($related_posts) {
        ...
        if ($first_related_post) {
            $first_link = '
            <div class="inline-related-box d-flex flex-row gap-2 align-items-center py-3">
                <h4 class="releated-head">همچنین بخوانید: </h4>
                <a href="' . get_permalink($first_related_post->ID) . '"> ' . get_the_title($first_related_post->ID) . '</a>
            </div>';
            $paragraphs[$ads_pos_1] .= $first_link;
        }
        if ($second_related_post) {
            $sec_link = '
            <div class="inline-related-box d-flex flex-row gap-2 align-items-center py-3">
                <h4 class="releated-head">همچنین بخوانید: </h4>
                <a href="' . get_permalink($second_related_post->ID) . '"> ' . get_the_title($second_related_post->ID) . '</a>
            </div>';
            $paragraphs[$ads_pos_2] .= $sec_link;
        }
        if ($third_related_post) {
            $third_link = '
            <div class="inline-related-box d-flex flex-row gap-2 align-items-center py-3">
                <h4 class="releated-head">همچنین بخوانید: </h4>
                <a href="' . get_permalink($third_related_post->ID) . '"> ' . get_the_title($third_related_post->ID) . '</a>
            </div>';
